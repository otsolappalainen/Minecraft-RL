...
class CustomCNNFeatureExtractor(BaseFeaturesExtractor):
    ...
    Splits the image into:
      - A "global" CNN branch (full 120x240)
      - A "center" CNN branch (the middle 64x64)
    Then fuses both branches into 256 dims total, merges with scalar input, 
    and outputs final features_dim (default 512).
    """
    def __init__(self, observation_space, features_dim=512):
        super().__init__(observation_space, features_dim)

        # ----- Global branch (full 120x240) -----
        self.img_head_global = nn.Sequential(
        ...
        # ----- Center branch (64x64 patch) -----
        self.img_head_center = nn.Sequential(
        ...
        # Compute shape of each branch
        with th.no_grad():
            dummy_full = th.zeros(1, 3, 120, 240)
            dummy_center = th.zeros(1, 3, 64, 64)
            out_global = self.img_head_global(dummy_full)
            out_center = self.img_head_center(dummy_center)
            self.global_features_dim = out_global.shape[1]
            self.center_features_dim = out_center.shape[1]
        ...
        self.center_dense = nn.Sequential(
            nn.Linear(self.center_features_dim, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 128),
            nn.ReLU(inplace=True),
        )

        # Merge the two CNN outputs down to 256

        # ----- Scalar part (hand + mobs + player_state + surrounding_blocks) -----
        scalar_dim = (
        ...
        # We'll keep the same 12 dims for surrounding_blocks
        total_scalar_input = 12 + scalar_dim
        self.state_fusion = nn.Sequential(
            nn.Linear(total_scalar_input, 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, 128),
            nn.ReLU(inplace=True),
        )

        # Final fusion to features_dim
    ...
    def forward(self, observations):
        # Full (120x240)
        global_features = self.img_head_global(observations["image"])

        # Crop the center 64x64 patch
        # For a 120x240 image: 
        #   vertical center ~ (120 // 2) = 60  =>  60-32=28 to 60+32=92
        #   horizontal center ~ (240 // 2)=120 => 120-32=88 to 120+32=152
        center_patch = observations["image"][:, :, 28:92, 88:152]
        center_features = self.img_head_center(center_patch)

        # Fuse both image branches into 256
        global_processed = self.global_dense(global_features)
        center_processed = self.center_dense(center_features)

        # img_features = global_processed + center_processed
        img_features = th.cat([global_processed, center_processed], dim=1)


        # Combine state inputs
        state_input = th.cat([
            observations["surrounding_blocks"],  # shape [N, 12]
            observations["hand"],
            observations["mobs"],
            observations["player_state"]
        ], dim=1)
        state_features = self.state_fusion(state_input)

        # Final fusion to produce the final features
...
def create_new_model(env):
    policy_kwargs = dict(
        ...
        net_arch=[256],
        lstm_hidden_size=256,  # This defines the LSTM memory size
